...
- **ADR 0010 (Manejo Dinámico Online/Offline, Acumulado en SQLite y Gráficos Apilados Barras/Líneas)**: Actualización automática del estado `is_online` a `false` ante fallos de conexión TCP y `true` en reconexiones exitosas. Incorporación de la columna `monthly_accumulated` en la tabla `samples` de SQLite (reiniciada por mes `YYYY-MM`). Rediseño de `FlowChart.tsx` a dos gráficos apilados verticalmente (Gráfico 1: Caudal por intervalo en Barras, Gráfico 2: Acumulado mensual en Líneas con puntos de datos visibles), con límites estrictos en el eje X (`min`/`max`) y paleta de 8 colores de alto contraste.
- **ADR 0011 (Calendario Híbrido, Eliminación del Slider DataZoom, Gradiente en Acumulados, Carga Offline y Comando Debug)**: Integración de widgets de calendario nativo junto a los inputs de texto `YYYY/MM/DD HH:mm` en `DateRangePicker.tsx`. Eliminación completa del slider inferior de dataZoom para resolver el overflow en el contenedor de gráficos y renderizado del gráfico de acumulado en líneas limpias con gradiente suave (`areaStyle` sin datapoints recargados). Carga inmediata de datos SQLite previa al sync TCP para renderizar gráficos cuando el dispositivo está Offline. Comando IPC y botón Debug "Recalcular DB" para recalcular el acumulado mensual en SQLite bajo demanda.
